...
static VALID_FIELDS: [&'static str; 14] = [
    "cmd",
    "workingdir",
    "autostart",
    "env",
...
impl Config {
    ...
    pub fn new(name: &str, argv: &str, config: &Yaml) -> Self {
        ...
        let exitcodes = match (&config["exitcodes"]).as_vec() {
            Some(v) => Some(
                v.iter()
                    .map(|a| {
                        a.as_i64()
                            .expect("Expected numeric value for field exitcode")
                    })
                    .collect(),
            ),
        ...
        let stopsignal = match to_str(config, "stopsignal") {
            Some(slice) => {
                self::Config::parse_signal(slice).expect("Expected signal for field signal")
            }
            None => SIGTERM,
        };
    ...
    pub fn fatal_cmp(&self, other: &Config) -> bool {
        if self.name != other.name || self.argv != other.argv || self.workingdir != other.workingdir
            || self.env != other.env || self.stdout != other.stdout
            || self.stderr != other.stderr || self.umask != other.umask
        {
            return true;
        }
        false
