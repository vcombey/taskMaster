    ...
    pub fn from_str(chunk: &str) -> Result<Target, ParseError> {
        ...
        match (chunk_split.get(0), chunk_split.get(1), chunk_split.get(2)) {
            ...
            (Some(name1), Some(name2), None) => match usize::from_str_radix(name2, 10) {
                ...
                Err(_) => Ok(Target::ServiceProcess((
                    name1.to_string(),
                    name2.to_string(),
                    None,
                ))),
            ...
            (Some(service_name), Some(process_name), Some(thread_id)) => {
                match usize::from_str_radix(thread_id, 10) {
                    Ok(id) => Ok(Target::ServiceProcess((
...
impl error::Error for ParseError {
    fn description(&self) -> &str {
        match *self {
            ...
            ParseError::BadThreadId => "bad parsing for thread id : must be a valid usize",
...
pub mod test_cmd {
    ...
    fn test_cmd_simple_process_r() {
        assert_eq!(
            Cmd::from_vec(vec!["start", "process_name"]).unwrap(),
            Cmd {
                instruction: Instruction::START,
    ...
    fn test_cmd_no_process_name() {
        ...
        assert_eq!(
            Cmd::from_vec(vec!["start", "service_name:"]),
    ...
    fn test_cmd_simple_service_process() {
        assert_eq!(
            Cmd::from_vec(vec!["start", "service_name:process_name"]).unwrap(),
            Cmd {
                instruction: Instruction::START,
                target_vec: vec![
    ...
    fn test_cmd_many_process() {
        assert_eq!(
            Cmd::from_vec(vec!["start", "process_one", "process_two"]).unwrap(),
            Cmd {
                instruction: Instruction::START,
                target_vec: vec![
    ...
    fn test_cmd_all() {
        assert_eq!(
            Cmd::from_vec(vec!["start", "all"]).unwrap(),
            Cmd {
                instruction: Instruction::START,
                target_vec: vec![Target::ALL],
            }
    ...
    fn test_cmd_too_many_level() {
        assert_eq!(
            Cmd::from_vec(vec!["start", "1:2:3:4"]),
            Err(ParseError::ToManyLevels)
        );
    }
    ...
    fn test_cmd_mix() {
        eprintln!(
            "{:?}",
            Cmd::from_vec(vec!["start", "process_one", "service_one:process_two"])
        );
        assert_eq!(
            Cmd::from_vec(vec!["start", "process_one", "service_one:process_two"]).unwrap(),
            Cmd {
                instruction: Instruction::START,
                target_vec: vec![
                    Target::Process("process_one".to_string(), None),
